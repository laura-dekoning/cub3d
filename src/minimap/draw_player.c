...
void	draw_rays(t_game *game, int colour)
{
	...
	while (i < NUMB_RAYS)
	{
		dir.x = cos(angle);
		dir.y = sin(angle);
		end_pos.x = (start_pos.x + dir.x * (game->ray[i].distance / GRIDSIZE) * game->minimap.grid_size);
		end_pos.y = (start_pos.y + dir.y * (game->ray[i].distance / GRIDSIZE) * game->minimap.grid_size);
...
void	draw_arrow_head(t_game *game,  t_vector_f start_pos, t_vector_f end_pos, t_vector_f dir, int colour)
{
	float	arrow_length;
	float	arrow_angle;
	...
	draw_line(game->minimap_image, end_pos, start_pos, colour);
...
void	draw_arrow(t_game *game, int colour)
{
	t_vector_f 	start_pos;
	t_vector_f 	end_pos;
	t_vector_f 	dir;
